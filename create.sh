...
function create_plugin_module() {
  ...
  if [ -z $1 ]; then
      ...
      创建一个独立Plugin module，名字任意：
          create_plugin_module <your_plugin_module_name>
EOF
      exit 1
  fi
...
"""
    ...
    # 创建项目目录
    PROJ=$1
    PLUGIN_ID='org.example.greeting'

    # 创建Plugin
    mkdir -p $PROJ/src/main/java/org/example/greeting
    echo $PLUGIN_TEMPLATE \
    > $PROJ/src/main/java/org/example/greeting/GreetingPlugin.java

    # 创建一个任务
    echo $TASK_TAMPLATE \
    > $PROJ/src/main/java/org/example/greeting/Greeting.java
    ...
    echo "implementation-class=org.example.greeting.GreetingPlugin" \
    > $PROJ/src/main/resources/META-INF/gradle-plugins/org.example.greeting.properties
...
> build.gradle
mkdir -p app
echo "apply plugin: 'org.example.greeting'" > app/build.gradle
}
